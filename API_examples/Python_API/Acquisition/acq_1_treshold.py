...
trig_dly = 8192                     # Trigger delay in samples
...
channels = [rp.RP_CH_1, rp.RP_CH_2]     # Generator output channels
...
if ENABLE_GENERATION:
    ...
    for channel in channels:
        ...
        print(f"{channel} configured: {waveform}, {freq} Hz, {ampl} V")
...
if ENABLE_GENERATION:
    ...
    rp.rp_GenTriggerOnly(channels[0])  # Trigger the first generator channel
...
# Get raw ADC data
res = rp.rp_AcqGetDataRawWithCalibNP(channels[0], trig_pos, data_ch_raw)

# Get voltage data
res = rp.rp_AcqGetDataVNP(channels[0], trig_pos, data_ch_V)

# Assign to variables for clarity
print(f"Retrieved {buff_size} samples from CH{channels[0]}")
