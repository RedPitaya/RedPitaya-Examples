...
def main():
    ...
    try:
        ...
        tx_buff_1 = np.array(data_int_1, dtype=np.uint8)  # Buffer for message 1
        print(f"    TX Buffer 1: {data_size_1} bytes - \"{test_message_1}\"")
        
        # Prepare second message buffer
        tx_buff_2 = np.array(data_int_2, dtype=np.uint8)  # Buffer for message 2
        ...
        # Configure message 0
        rp_hw.rp_SPI_SetBufferForMessage(0, tx_buff_1, True, toggle_cs_msg1)
        print(f"    Message 0: {data_size_1} bytes, Toggle CS: {toggle_cs_msg1}")
        
        # Configure message 1
        rp_hw.rp_SPI_SetBufferForMessage(1, tx_buff_2, True, toggle_cs_msg2)
        ...
        print(f"    RX Buffer 0: {res_0[1].size} bytes - {res_0[1]}")
        data_rx_0 = res_0[1].tobytes().decode('utf-8')
        print(f"    RX Message 0: \"{data_rx_0}\"")

        # Read RX buffer 1
        res_1 = rp_hw.rp_SPI_GetRxBuffer(1)
        print(f"    RX Buffer 1: {res_1[1].size} bytes - {res_1[1]}")
        data_rx_1 = res_1[1].tobytes().decode('utf-8')
        print(f"    RX Message 1: \"{data_rx_1}\"")
