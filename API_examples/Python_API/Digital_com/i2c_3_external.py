...
def main():
    ...
    try:
        ...
        print(f"    Device address: 0b{res[1]:07b}")
        
        # ===== STEP 4: Prepare Write Buffer =====
        print("\n[4] Preparing write buffer...")
        rx_buff = np.zeros(PAGE_SIZE + 2, dtype=np.uint8)  # Buffer for address + data
        ...
        rx_buff[2:2 + data_size] = data_int  # Copy data to buffer
        ...
        res = rp_hw.rp_I2C_IOCTL_WriteBuffer(rx_buff[0:2]) #, 2)
        ...
        res = rp_hw.rp_I2C_IOCTL_ReadBuffer(rx_buff) #, data_size)
        if res != 0:
        ...
        data_read = rx_buff[:data_size].tobytes().decode('utf-8')
        ...
        data_read_after = rx_buff[:data_size].tobytes().decode('utf-8')
        print(f"    Read successful (result: {res})")
        print(f"    Read data: \"{data_read_after}\"")
        print("    (This shows data at different EEPROM location)")
