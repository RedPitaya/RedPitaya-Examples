#!/usr/bin/python3
"""
Red Pitaya DAC Streaming - One Buffer Mode

Loads the entire waveform into a single FPGA RAM buffer upfront, then plays
it from there. The board loops over the same buffer without requesting
additional data from the host.

Use this mode when the waveform fits in FPGA RAM and does not change
during playback. Playback is deterministic and starts with minimal latency
once the buffer is loaded.

Mode: One buffer (preload to FPGA RAM)
Channels: 2
Bit depth: 16-bit
"""
...
waveform_samples = 1024 * 32    # Number of samples in generated waveform

print("=" * 70)
print("Red Pitaya DAC Streaming Configuration - Memory Mode")
print("=" * 70)
...
print(f"Mode:            Direct memory streaming (no WAV file)")
print("=" * 70)


# ==============================================================================
# WAVEFORM GENERATION - Create the signal data
# ==============================================================================
...
# Convert to int16 format (16-bit signed integer)
waveform_data = data.astype(np.int16).tolist()
print(f"Generated {len(waveform_data)} samples (16-bit format)")
...
# Step 4: Load waveform data into FPGA memory for both channels
print("\nLoading waveform data into FPGA memory...")
client.setMemory16Bit(1, waveform_data)  # Load data to channel 1 memory
client.setMemory16Bit(2, waveform_data)  # Load data to channel 2 memory
print("Waveform loaded to both DAC channels")

# Step 5: Start streaming from memory
print("\nStarting DAC streaming from memory...")
host = confObj.getHosts()[0]
if not client.startStreamingFromMemory(host):
    print("ERROR: Failed to start DAC streaming from memory")
    exit(1)

print("Streaming started - outputting waveform to both DACs from memory...")
