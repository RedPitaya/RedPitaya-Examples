/* @brief This is a simple application for testing SPI communication on a
 * RedPitaya
 *
...
int main(int argc, char *argv[]) {
  ...
  res = rp_SPI_SetMode(RP_SPI_MODE_LIST); // Set SPI mode: Low idle level,
                                          // sample on trailing edge.
  printf("Set mode: %d\n", res);

  rp_SPI_SetCSMode(RP_SPI_CS_HIGH);
  ...
  size_t rx_size = 0;
  res = rp_SPI_GetRxBuffer(0, &rx_buffer,
                           &rx_size); // Get pointer to rx buffer. No need free
                                      // buffer. Api itself destroy buffer.
