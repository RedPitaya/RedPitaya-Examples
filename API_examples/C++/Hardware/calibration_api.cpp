...
int main(int argc, char *argv[]) {
  ...
  int32_t offset;
  res =
      rp_CalibGetFastADCCalibValue(RP_CH_1_CALIB, RP_DC_CALIB, &gain, &offset);
  printf("Calibration factors for channel 1 GAIN %f OFFSET %d result = %d\n",
         gain, offset, res);
  res =
      rp_CalibGetFastADCCalibValue(RP_CH_2_CALIB, RP_DC_CALIB, &gain, &offset);
  ...
  // Returns the default calibration for the current board version. It may be
