/* @brief This is a simple application for testing CAN communication on a
 * RedPitaya
 *
...
int main(int argc, char *argv[]) {
  ...
  int res = rp_CanSetFPGAEnable(
      true); // init can in fpga for pass can controller to GPIO (N7,P7)
  ...
  res = rp_CanSetControllerMode(RP_CAN_0, RP_CAN_MODE_LOOPBACK,
                                false); // set loopback mode
  ...
  res = rp_CanSetControllerMode(RP_CAN_1, RP_CAN_MODE_LOOPBACK,
                                false); // set loopback mode
  printf("Set loopback mode OFF: %d\n", res);
  ...
  res = rp_CanSend(RP_CAN_0, 123, tx_buffer, 3, false, false,
                   0); // write buffer to can0
  printf("Write result: %d\n", res);

  res = rp_CanSend(RP_CAN_0, 321, tx_buffer, 5, true, false,
                   0); // write buffer to can0
  printf("Write result: %d\n", res);
