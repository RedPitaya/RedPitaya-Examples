...
int main(int argc, char **argv) {
  ...
  auto adc_axi_start2 = adc_axi_size + bufferSize * 2;
  if (rp_GenAxiReserveMemory(RP_CH_2, adc_axi_start2,
                             adc_axi_start2 + bufferSize) != RP_OK) {
    fprintf(stderr, "Error setting address for DMA mode for OUT2\n");
    return 1;
  }

  if (rp_GenAxiSetDecimationFactor(RP_CH_1, 2) != RP_OK) {
    fprintf(stderr, "Error setting decimation for generator\n");
    return 1;
  }

  if (rp_GenAxiSetDecimationFactor(RP_CH_2, 2) != RP_OK) {
    fprintf(stderr, "Error setting decimation for generator\n");
    return 1;
  }
