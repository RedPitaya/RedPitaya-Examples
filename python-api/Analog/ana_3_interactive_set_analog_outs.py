...
Important Note:
    ...
    Output impedance: 50Ω, Maximum current: ±10mA per output
...
def parse_and_validate_voltages(input_str):
    ...
    try:
        ...
        if len(voltages) == 1:
            voltages = voltages * num_analog_outputs
            print(f"Setting all {num_analog_outputs} outputs to {voltages[0]:.3f}V")
        
        # Handle four values (one per output)
        elif len(voltages) == num_analog_outputs:
            print(f"Setting individual voltages for {num_analog_outputs} outputs")
        
        # Invalid number of values
        else:
            print(f"ERROR: Expected 1 or {num_analog_outputs} values, got {len(voltages)}")
            print(f"       Using default voltage {default_voltage}V for all outputs")
            voltages = [default_voltage] * num_analog_outputs
        
        # Validate and clamp voltages to valid range
        validated_voltages = []
        for i, v in enumerate(voltages):
            if not min_voltage <= v <= max_voltage:
                print(f"WARNING: AO[{i}] voltage {v:.3f}V is out of range ({min_voltage}-{max_voltage}V)")
                v = max(min_voltage, min(max_voltage, v))
                print(f"         Clamped to {v:.3f}V")
            validated_voltages.append(v)
        
        return True, validated_voltages
...
def set_output_voltages(voltages):
    """
    Set the output voltages on all analog outputs.
    
    Args:
        voltages: List of 4 voltage values to set
    """
    print("-" * 70)
    for i in range(num_analog_outputs):
        # Set the output voltage using generic analog pin function
        rp.rp_ApinSetValue(analog_out[i], voltages[i])
        print(f"Set voltage on AO[{i}] to {voltages[i]:.3f} V")
    print("-" * 70)
...
try:
    while True:
        ...
        success, voltages = parse_and_validate_voltages(user_input)
        
        if success and voltages:
            # Set the output voltages
            set_output_voltages(voltages)
...
try:
    while True:
        ...
        if success and voltages:
            ...
            print("-" * 70)
            for i in range(num_analog_outputs):
                rp.rp_AOpinSetValue(i, voltages[i])
                print(f"Set voltage on AO[{i}] to {voltages[i]:.3f} V")
            print("-" * 70)
            print("✓ Voltages set successfully")
        else:
