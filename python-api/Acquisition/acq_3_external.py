#!/usr/bin/python3
"""
Red Pitaya Acquisition with External Trigger Example
=====================================================

This example demonstrates data acquisition triggered by an external trigger signal
on Red Pitaya. The acquisition waits for an external trigger pulse (positive or
negative edge) before capturing data. This is useful for synchronizing acquisition
with external events, instruments, or timing signals.

Features:
- External trigger-based acquisition
- Configurable trigger edge (positive or negative)
- Configurable trigger level and delay
- Captures data synchronized with external events
- Data output in volts

Hardware Requirements:
- Red Pitaya board (STEMlab 125-14 or similar)
- External trigger source connected to EXT trigger input (DIO0_P pin on E2 connector)
- Optional: Signal source connected to IN1 for data capture
...
Usage:
    python acq_3_external.py
    
    Connect external trigger signal to DIO0_P pin.
    Connect signal to measure to IN1.
    The program will wait for external trigger before capturing data.

Configuration:
    Modify parameters in the CONFIGURATION section:
    - acq_trig_sour: External trigger edge (EXT_PE or EXT_NE)
    - trig_lvl: Trigger threshold voltage (for external trigger)
    - dec: Decimation factor (affects sample rate)
    - buff_size: Number of samples to capture

Pinout Reference:
    External trigger input: DIO0_P (Extension connector E2, pin 19)
    Trigger voltage range: 0-3.3V (CMOS/TTL compatible)
...
# ==============================================================================
# CONFIGURATION - Set your external trigger acquisition parameters
# ==============================================================================
...
trig_lvl = 0.5                  # Trigger threshold level in volts (for external trigger)
trig_dly = 0                    # Trigger delay in samples

print("=" * 70)
print("Red Pitaya Acquisition with External Trigger Configuration")
print("=" * 70)
...
print(f"Trigger source:      {acq_trig_sour} (External trigger)")
print(f"Trigger level:       {trig_lvl} V")
print(f"Trigger delay:       {trig_dly} samples")
print()
print("External Trigger Connection:")
print("  Input pin:         DIO0_P (E2 connector, pin 19)")
print("  Voltage range:     0-3.3V (CMOS/TTL compatible)")
print("  Trigger edge:      " + ("Positive (rising)" if "PE" in str(acq_trig_sour) else "Negative (falling)"))
print()
print("NOTE: Acquisition waits for external trigger pulse")
print("=" * 70)
...
# ==============================================================================
# TRIGGER AND CAPTURE - Wait for external trigger and capture data
# ==============================================================================

print("\nWaiting for external trigger event...")
print("Apply trigger pulse to DIO0_P (E2 connector, pin 19)")
