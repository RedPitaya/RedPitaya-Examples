...
This example demonstrates DMA (Direct Memory Access) data acquisition on Red Pitaya
with data output to a text file. It captures data from two input channels simultaneously
and writes the raw ADC values to a file for later analysis.
...
- Automatic file output of captured data
...
Configuration:
    ...
    - data_size: Total number of samples to capture per channel
    - read_chunk_size: Number of samples to read per iteration
