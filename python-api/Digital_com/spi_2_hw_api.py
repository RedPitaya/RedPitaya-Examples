...
# ioctl command codes
SPI_IOC_WR_MODE = fcntl.IOC(fcntl.IOC_WRITE, SPI_IOC_MAGIC, 1, 1)
SPI_IOC_RD_MODE = fcntl.IOC(fcntl.IOC_READ, SPI_IOC_MAGIC, 1, 1)
SPI_IOC_WR_MAX_SPEED_HZ = fcntl.IOC(fcntl.IOC_WRITE, SPI_IOC_MAGIC, 4, 4)
SPI_IOC_RD_MAX_SPEED_HZ = fcntl.IOC(fcntl.IOC_READ, SPI_IOC_MAGIC, 4, 4)
...
def SPI_IOC_MESSAGE(n):
    ...
    return fcntl.IOC(fcntl.IOC_WRITE, SPI_IOC_MAGIC, 0, n * ctypes.sizeof(spi_ioc_transfer))
...
def read_flash_id(fd):
    ...
    try:
        # Prepare buffers
        buf0 = bytearray([0x9F])  # RDID command
        buf1 = bytearray([0x01, 0x23, 0x45])  # Sample loopback data
        
        # Create transfer structures
        xfer0 = spi_ioc_transfer()
        xfer0.tx_buf = ctypes.cast(ctypes.c_char_p(bytes(buf0)), ctypes.c_void_p).value
        xfer0.rx_buf = ctypes.cast(ctypes.c_char_p(buf0), ctypes.c_void_p).value
        xfer0.len = len(buf0)
        
        xfer1 = spi_ioc_transfer()
        xfer1.tx_buf = ctypes.cast(ctypes.c_char_p(bytes(buf1)), ctypes.c_void_p).value
        xfer1.rx_buf = ctypes.cast(ctypes.c_char_p(buf1), ctypes.c_void_p).value
        xfer1.len = len(buf1)
