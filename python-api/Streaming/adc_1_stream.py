#!/usr/bin/python3
"""
Red Pitaya ADC Streaming Example

This example demonstrates how to collect ADC data from Red Pitaya using the streaming API.
It captures data from two channels simultaneously and stores them in numpy arrays for analysis.
"""
...
obj.setReciveDataFunction(callback.__disown__())  # __disown__() transfers ownership to client
obj.setVerbose(False)  # Disable detailed logging
...
obj.sendConfig('adc_pass_mode', 'NET')              # Set network mode (NET or FILE for SD card)
obj.sendConfig('adc_decimation', f'{decimation}')   # Set decimation factor
obj.sendConfig('block_size', f'{block_size}')       # Set network packet size
obj.sendConfig('adc_size', f'{adc_size}')           # Set FPGA buffer size
obj.sendConfig('channel_state_1', f'{ch1_state}')   # Enable/disable channel 1
obj.sendConfig('channel_state_2', f'{ch2_state}')   # Enable/disable channel 2

# Step 4: Start streaming
print("\nStarting data acquisition...")
if not obj.startStreaming():
