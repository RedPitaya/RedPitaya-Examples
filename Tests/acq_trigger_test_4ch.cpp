...
auto printHelp(char *prog) -> void {
  std::cout << prog
            << "-h SCPI_SERVER_IP [-t1] [-t2] [-t3] [-a] [-d] [-b] [-v] [-s]\n";
  ...
  if (dac) {
    ...
    std::cout << "For test need connect REF Sine signal 500 kHz 1V p-p -> IN1, "
                 "IN2, IN3, IN4\n";
  }
...
auto startGenerator(std::string ip, rp_channel_t _channel, rp_waveform_t _wave,
                    uint32_t _rate, float _volt, float _offset,
                    bool _verbose) -> int {
  ...
  if (_verbose) {
    ...
    string s = "* Start signal generator. Ampl: " + to_string(_volt) +
               " Vpp. Offset: " + to_string(_offset) +
               " V. Freq:  " + to_string(_rate) + " Hz. Wave: " + wave + " " +
               (ret == 0 ? green.color("[OK]") : red.color("[FAIL]"));
    std::cout << s << "\n";
  }
...
auto getData(rp_channel_trigger_t _ch, uint32_t _dec, int32_t _triggerDelay,
             float _trigLevel, rp_acq_trig_src_t _ts, bool _verbose,
             buffers_t *_buffer) -> int {
...
auto getDataSplit(testStepSplit settings, uint32_t _dec, int32_t _triggerDelay,
                  float _trigLevel, bool _verbose, buffers_t *_buffer) -> int {
  static double rate = getADCRate();
  Color::Modifier red(Color::FG_RED);
  Color::Modifier green(Color::FG_GREEN);
  ...
  for (int ch = 0; ch < 4; ch++) {
    if (chEnable[ch]) {
      ...
      while (pretrigger < preTriggerWait && pretrigger < ADC_BUFFER_SIZE) {
        ...
        if (timeout < getClock()) {
          printf(
              "Fail pre trigger counter on Ch %d. Current counter %d need %d\n",
  ...
  for (int ch = 0; ch < 4; ch++) {
    if (chEnable[ch]) {
      ...
      ret |= rp_AcqGetDataRaw((rp_channel_t)ch, trig_pos, &size,
                              _buffer->ch_i[ch]);
    }
  }
...
auto testTrig(settings s) -> int {
  ...
  auto buffer =
      rp_createBuffer(getADCChannels(), ADC_BUFFER_SIZE, true, false, false);
  if (!buffer) {
  ...
  for (auto &i : test_list) {
    ...
    for (auto freq : freq_list) {
      int testResult = 0;
      string testName = "Trigger position test. Decimate: " + to_string(dec) +
                        ". Signal freq: " + to_string(freq);
      if (s.verbose) {
        std::cout << testName << "\n";
      }
      testResult |= startGenerator(s.ip, RP_CH_1, RP_WAVEFORM_SINE, freq, 0.9,
                                   0, s.verbose);

      for (auto test_step : trig_list) {
        testResult |= getData(test_step.channel_t, dec, ADC_BUFFER_SIZE / 2.0,
                              0, test_step.t_source, s.verbose, buffer);
        ...
        if (sign >= 0) {
          if (!(buffer->ch_i[ch][0] > buffer->ch_i[ch][end] &&
                buffer->ch_i[ch][0] >= 0 && buffer->ch_i[ch][end] < 0)) {
            bufferIsOk = false;
          }
        } else {
          if (!(buffer->ch_i[ch][0] < buffer->ch_i[ch][end] &&
...
auto testTrigSettingNow(settings s) -> int {
  ...
  result |=
      startGenerator(s.ip, RP_CH_1, RP_WAVEFORM_SINE, 1000, 0.9, 0,
                     s.verbose); // low frequency for very high decimations
  result |= rp_AcqReset();
  ...
  for (auto dec : dec_list) {
    ...
    bool posNotChanged =
        trig_pos == trig_pos_prev || write_pos == write_pos_prev;

    if (posNotChanged) {
      std::cout << "Position was not changed\n";
      std::cout << "Trigger position " << trig_pos << " position in old test "
                << trig_pos_prev << "\n";
      std::cout << "Write position " << write_pos << " position in old test "
                << write_pos_prev << "\n";
    }
    ...
    if (testResult) {
      std::cout << "Required trigger position " << trig_pos << " after set now "
                << new_trig_pos << "\n";
      std::cout << "Required write position " << write_pos << " after set now "
                << new_write_pos << "\n";
    }
...
auto testKeepArm(settings s) -> int {
  ...
  result |=
      startGenerator(s.ip, RP_CH_1, RP_WAVEFORM_SINE, genFreq, 0.9, 0,
                     s.verbose); // low frequency for very high decimations
  result |= rp_AcqReset();
  result |= rp_AcqSetTriggerLevel(RP_T_CH_1, 0);
  result |= rp_AcqSetTriggerHyst(0.005);
  ...
  for (auto dec : dec_list) {
    ...
    if (trig_pos == prev_test_trig) {
      ...
      std::cout << "Trigger position " << write_pos << " prev test position "
                << write_pos_prev << "\n";
      ret |= true;
    }
    ...
    while (readCount) {
      ...
      if (write_pos == write_pos_prev) {
        ...
        std::cout << "Write position " << write_pos << " prev position "
                  << write_pos_prev << "\n";
        ret |= true;
      }
      ...
      if (trig_pos != trig_pos_prev) {
        ...
        std::cout << "Trigger position " << write_pos << " prev position "
                  << write_pos_prev << "\n";
        ret |= true;
      }
...
auto testTrigDelay(settings s) -> int {
  ...
  auto buffer =
      rp_createBuffer(getADCChannels(), ADC_BUFFER_SIZE, true, false, true);
  if (!buffer) {
    printf("Can't allocate buffer\n");
    exit(-1);
  }
  ...
  for (auto dec : dec_list) {
    for (uint32_t i = 1; i <= 1024; i *= 2) {
      string testName = "Testing trigger delay. Delay: " + to_string(i) +
                        " Decimate: " + to_string(dec);
      if (s.verbose) {
        std::cout << testName << "\n";
      }
      ...
      int ret = 0;
      ret |=
          startGenerator(s.ip, RP_CH_1, RP_WAVEFORM_DC, 1000, 0, 0, s.verbose);
      // Clean buffer in FPGA
      usleep(1000);
      ret |= getData(RP_T_CH_1, 1, ADC_BUFFER_SIZE, 0, RP_TRIG_SRC_NOW,
                     s.verbose, buffer);
      ret |= getData(RP_T_CH_1, 1, ADC_BUFFER_SIZE, 0, RP_TRIG_SRC_NOW,
                     s.verbose, buffer);
      ...
      ret |= rp_AcqReset();
      ret |= rp_AcqSetDecimationFactor(dec);
      ret |= rp_AcqSetTriggerLevel(
          RP_T_CH_1,
          0.7); // need low threshold so the very first sample can be high
      ret |= rp_AcqSetTriggerHyst(0.005);
      ...
      ret |= rp_AcqSetTriggerSrc(RP_TRIG_SRC_CHA_PE);
      ret |=
          startGenerator(s.ip, RP_CH_1, RP_WAVEFORM_DC, 1000, 1, 0, s.verbose);
      ...
      for (uint32_t z = 0; z < buffer->size; z++) {
        if (buffer->ch_i[0][z] >=
            (1 << (bits - 1)) * 0.7) { // must take into account the rise time
                                       // and initial ringing of step signal
          sampWithData++;
        }
      }
      ...
      if (testResult) {
        std::cout
            << "The number of data samples is not equal to the trigger delay\n";
...
auto testNoise(settings s) -> int {
  ...
  for (auto &i : test_list) {
    ...
    for (auto freq : freq_list) {
      int testResult = 0;
      string testName = "Noise test. Decimate: " + to_string(dec) +
                        ". Signal freq: " + to_string(freq);
      if (s.verbose) {
        std::cout << testName << "\n";
      }
      ...
      for (auto test_step : trig_list) {
        testResult |= getData(test_step.channel_t, dec, ADC_BUFFER_SIZE, 0,
                              test_step.t_source, s.verbose, buffer);
...
auto testSplitMode(settings s) -> int {
  ...
  vector<testStepSplit> trig_list = {
      {.t_source = {RP_TRIG_SRC_CHA_PE, RP_TRIG_SRC_CHB_PE, RP_TRIG_SRC_CHC_PE,
                    RP_TRIG_SRC_CHD_PE}},
      {.t_source = {RP_TRIG_SRC_CHA_PE, RP_TRIG_SRC_CHB_NE, RP_TRIG_SRC_CHC_PE,
                    RP_TRIG_SRC_CHD_NE}},
      {.t_source = {RP_TRIG_SRC_CHA_NE, RP_TRIG_SRC_CHB_NE, RP_TRIG_SRC_CHC_NE,
                    RP_TRIG_SRC_CHD_NE}},
      {.t_source = {RP_TRIG_SRC_CHA_NE, RP_TRIG_SRC_CHB_PE, RP_TRIG_SRC_CHC_NE,
                    RP_TRIG_SRC_CHD_PE}},
      {.t_source = {RP_TRIG_SRC_CHA_NE, RP_TRIG_SRC_DISABLED,
                    RP_TRIG_SRC_CHC_NE, RP_TRIG_SRC_DISABLED}},
      {.t_source = {RP_TRIG_SRC_DISABLED, RP_TRIG_SRC_CHB_PE,
                    RP_TRIG_SRC_DISABLED, RP_TRIG_SRC_CHD_PE}},
      {.t_source = {RP_TRIG_SRC_CHA_NE, RP_TRIG_SRC_DISABLED,
                    RP_TRIG_SRC_CHC_PE, RP_TRIG_SRC_DISABLED}}
      //,
      //    {.t_source = {RP_TRIG_SRC_CHA_PE, RP_TRIG_SRC_CHA_PE,
      //    RP_TRIG_SRC_CHA_PE, RP_TRIG_SRC_CHA_PE}},
      //    {.t_source = {RP_TRIG_SRC_CHD_PE, RP_TRIG_SRC_CHD_PE,
      //    RP_TRIG_SRC_CHD_PE, RP_TRIG_SRC_CHD_PE}}
  };
  ...
  for (auto &i : test_list) {
    ...
    for (auto freq : freq_list) {
      ...
      for (auto test_step : trig_list) {
        testResult |= getDataSplit(test_step, dec, ADC_BUFFER_SIZE / 2.0, 0,
                                   s.verbose, buffer);
        printf("\t - Test ");
        for (int ch = 0; ch < 4; ch++) {
...
int main(int argc, char **argv) {
  ...
  fprintf(stderr, "\nTo test 4-channel mode, a second Red Pitaya with a "
                  "generator in SCPI mode is required. The connection should "
                  "be RP (Out1) -> RP 4ch (In1, In2, In3, In4)\n\n");
