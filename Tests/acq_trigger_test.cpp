...
auto startGenerator(rp_channel_t _channel, rp_waveform_t _wave, uint32_t _rate,
                    float _volt, float _offset, bool _verbose) -> int {
...
auto testTrig(settings s) -> int {
  ...
  for (auto &i : test_list) {
    ...
    for (auto freq : freq_list) {
      ...
      testResult |= startGenerator(RP_CH_1, RP_WAVEFORM_SINE, freq,
                                   getDACGainCh1() * 0.9, 0, s.verbose);

      testResult |= getData(RP_T_CH_1, dec, ADC_BUFFER_SIZE / 2.0, 0,
                            RP_TRIG_SRC_CHA_PE, s.verbose, buffer);
      ...
      testResult |= getData(RP_T_CH_1, dec, ADC_BUFFER_SIZE / 2.0, 0,
                            RP_TRIG_SRC_CHA_NE, s.verbose, buffer);
...
auto testTrigSettingNow(settings s) -> int {
  ...
  result |=
      startGenerator(RP_CH_1, RP_WAVEFORM_SINE, 1000, getDACGainCh1() * 0.9, 0,
...
auto testKeepArm(settings s) -> int {
  ...
  result |=
      startGenerator(RP_CH_1, RP_WAVEFORM_SINE, genFreq, getDACGainCh1() * 0.9,
                     0, s.verbose); // low frequency for very high decimations
  result |= rp_AcqReset();
  result |= rp_AcqSetTriggerLevel(RP_T_CH_1, 0);
  result |= rp_AcqSetTriggerHyst(0.005);
  result |= rp_AcqSetTriggerDelayDirect(ADC_BUFFER_SIZE / 2.0);
...
auto testNoise(settings s) -> int {
  ...
  for (auto &i : test_list) {
    ...
    for (auto freq : freq_list) {
      ...
      testResult |=
          startGenerator(RP_CH_1, RP_WAVEFORM_SINE, freq, 0.9, 0, s.verbose);
